...
from api.pappers_client import (
    PappersAPIError,
    fetch_pappers_company,
)
...
def test_fetch_company_with_siren_uses_direct_endpoint():
    ...
    session = DummySession([entreprise_payload])

    data = fetch_pappers_company("123456789", api_token="token", session=session)
    ...
    assert session.calls[0]["url"].endswith("entreprise")
...
def test_fetch_company_with_name_uses_search_then_entreprise():
    ...
    session = DummySession([search_payload, entreprise_payload])

    data = fetch_pappers_company("ACME", api_token="token", session=session)
    ...
    assert session.calls[0]["url"].endswith("recherche")
    assert session.calls[1]["url"].endswith("entreprise")


def test_missing_results_raise_value_error():
    session = DummySession([{"resultats": []}])

    with pytest.raises(ValueError):
        fetch_pappers_company("Unknown", api_token="token", session=session)


def test_empty_payload_raises_api_error():
    session = DummySession([{"resultats": [{"siren": "987654321"}]}, {}])

    with pytest.raises(PappersAPIError):
        fetch_pappers_company("ACME", api_token="token", session=session)
