...
def fetch_pappers_company(
    ...
    *,
    ...
    session:
        Optional ``requests.Session`` instance. Mainly useful for testing.
...
def _search_company_siren(http: requests.Session, token: str, query: str) -> str:
    response = http.get(
        PAPPERS_SEARCH_URL,
        params={
            "api_token": token,
...
def _fetch_company_data(http: requests.Session, token: str, siren: str) -> Dict[str, Any]:
    response = http.get(
